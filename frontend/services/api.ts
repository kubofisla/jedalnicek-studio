...
export interface BackendRecipe {
  ...
  ingredients: Array<{
    id: string; // ID reference to IngredientSuggestion
...
export const adaptBackendRecipe = (data: BackendRecipe): Recipe => {
  ...
  return {
    ...
    tags: {
      sk: Array.isArray(data.tags) ? data.tags : [],
      en: Array.isArray(data.tags) ? data.tags : []
    },
    ingredients: {
...
export const adaptBackendIngredient = (data: BackendIngredient): IngredientSuggestion => ({
  id: data.id.toString(),
  name: data.name,
  energy: data.energy,
      ...
      setTimeout(() => {
        ...
        const newBackendRecipe: BackendRecipe = {
          ...
          ingredients: recipeData.ingredients.map((i: any) => ({
            id: i.ingredientId || `gen-${Date.now()}-${Math.random()}`,
            name: i.name,
            quantity: i.quantity,
            unit: i.unit
          })),
