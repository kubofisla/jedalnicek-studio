...
import { ArrowLeft, Clock, Flame, List, CheckCircle, Activity, Droplet, Wheat, ListTodo } from 'lucide-react';
import RecipeCard from '../components/RecipeCard';

const RecipeDetail: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { recipes, t, language, toggleInventory, inventory } = useMealContext();
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-10">
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
        <div className="relative h-64 sm:h-80 md:h-96 w-full">
          ...
          <button
            onClick={() => toggleInventory(recipe.id)}
            className={`
              absolute top-6 right-6 flex items-center justify-center w-12 h-12 rounded-full shadow-lg backdrop-blur-md transition-all duration-200 z-10
              ${inventory.some(i => i.recipeId === recipe.id)
                ? 'bg-blue-500 text-white'
                : 'bg-white/90 text-gray-400 hover:text-blue-500 hover:bg-white'
              }
            `}
            title={inventory.some(i => i.recipeId === recipe.id) ? t('removeFromFridge') : t('addToFridge')}
          >
            ...
            <ul className="space-y-3 bg-gray-50 p-5 rounded-xl border border-gray-100">
              {recipe.ingredients[language].map((ing, idx) => (
                <li key={idx} className="flex items-start text-sm text-gray-700 font-medium leading-relaxed">
                  ...
                  <span className="font-mono whitespace-pre mr-2 text-gray-500 inline-block" style={{ minWidth: '5ch' }}>{ing.amount}</span>
                  <span>{ing.name}</span>
                </li>
              ))}
            </ul>
